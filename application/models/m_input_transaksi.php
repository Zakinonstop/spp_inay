...
class m_input_transaksi extends CI_Model
{
    ...
    public function get_tahun_tagihan($idnya_santri)
    {
        ...
        $this->db->join('tb_bulan', 'tb_data_tagihan.id_bulan = tb_bulan.id_bulan');
        $this->db->distinct();
        $this->db->select('nama_tahun');
        $query = $this->db->get_where('tb_data_transaksi', ['id_data_santri' => $idnya_santri])->result();
        return $query;
    }

    public function get_nama($idnya_santri)
    {
        return $this->db->get_where('tb_data_santri', ['id' => $idnya_santri])->result();
    }
