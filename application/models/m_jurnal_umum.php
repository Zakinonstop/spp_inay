...
class m_jurnal_umum extends CI_Model
{
	...
	// {
	// 	$this->db->join('tb_jurnal_umum', 'tb_jurnal_umum.id_transaksi = tb_jurnal_umum.id_transaksi');
	...
	public function get_tr_santri($id)
	{
		...
		return $this->db->get_where('tb_jurnal_umum', ['id_data_transaksi' => $id])->row();
	}
	...
	public function edit_save($data, $id)
	{
		...
		$this->db->where('id_data_transaksi', $id);
		$this->db->update('tb_jurnal_umum', $data);
	...
	{
		return $this->db->get_where('tb_jurnal_umum', ['id_transaksi' => $id])->row();
	}
